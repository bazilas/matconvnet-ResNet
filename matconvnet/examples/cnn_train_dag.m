...
for epoch=start+1:opts.numEpochs
  ...
  if numel(opts.gpus) <= 1
    [stats.train(epoch),prof, momentum] = process_epoch(net, state, opts, 'train') ;
    ...
    spmd
      ...
      [stats_.train, prof_, momentum_] = process_epoch(net_, state, opts, 'train') ;
      stats_.val = process_epoch(net_, state, opts, 'val') ;
      if labindex == 1, savedNet_ = net_.saveobj() ; end
    end
    net = dagnn.DagNN.loadobj(savedNet_{1}) ;
    momentum = momentum_{1};%it should be identical for all gpus
    ...
    clear net_ stats_ stats__ savedNet savedNet_ momentum_;
  end
...
% -------------------------------------------------------------------------
function [stats, prof, momentum] = process_epoch(net, state, opts, mode)
% -------------------------------------------------------------------------
